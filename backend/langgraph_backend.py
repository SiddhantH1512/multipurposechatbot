...
# print("\n=== FINAL TOOL INSPECTION BEFORE GRAPH USE ===")
# for tool in all_tools:  # or whatever list you pass to the agent/graph
#     name = getattr(tool, 'name', str(tool))
#     has_sync = hasattr(tool, '_run') and tool._run is not NotImplemented
#     has_async = hasattr(tool, '_arun') and tool._arun is not NotImplemented
#     sync_callable = has_sync and callable(tool._run)
#     print(f"  • {name:25}   sync: {has_sync}   callable sync: {sync_callable}   async: {has_async}")
#     if has_sync and not sync_callable:
#         print("      WARNING: _run exists but is NotImplemented")

# print("Creating ToolNode with these tools:")
# for t in all_tools:  # the list you pass
#     print(f"  - {t.name}: _run callable? {callable(getattr(t, '_run', None))}")
