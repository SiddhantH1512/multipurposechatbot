...
# ── Helper: format follow-up suggestions ─────────────────────────────────────
                ...
                for event in user_chatbot.stream(
                    {
                        ...
                        "conflict_note": "",
                        "is_calculation": False,        # gate node will re-detect
                        "is_conflict_question": False,  # gate node will re-detect
                    },
                    config=config,
                    stream_mode="values",
                ):
                    ...
                    # ── FIX #8: Corrected status indicator logic ──────────────
                    # Previous code only showed status on ToolMessage, meaning:
                    #   - "No relevant docs" never showed when need_retrieval=False
                    #   - Status was shown at wrong point in stream
                    # Fixed: check need_retrieval flag first, then ToolMessage
                    if not status_shown:
                        need_ret = event.get("need_retrieval")
                        skip_ret = event.get("skip_retrieval", False)
                        msgs     = event.get("messages", [])

                        if need_ret is False:
                            # Pure conversational turn — no document search needed
                            # Don't show any status; just let the answer come through
                            status_shown = True
                            ...
                            if (
                                ...
                                and getattr(last_msg, "name", "") == "rag_tool"
                            ):
                                if skip_ret:
