...
def build_rag_tool(user_department: str, user_role: str, tenant_id: str = "default"):
    ...
    def rag_tool(query: str) -> str:
        ...
        # ── Fetch authorised document pool ──────────────────────────────
        try:
            fetch_kwargs = {"k": 500}
            if pgvector_filter:
                fetch_kwargs["filter"] = pgvector_filter
        ...
        # ── FIX #4: Increased k from 6 → 8 for wider retrieval net ─────
        # Factoid answers buried in policy tables or edge-case sections were
        # being missed with k=6. Increasing to 8 gives the reranker more
        # candidates to work with at a negligible latency cost.
        retriever_kwargs: dict = {"k": 8}
        if pgvector_filter:
            retriever_kwargs["filter"] = pgvector_filter
        ...
        try:
            bm25_retriever = BM25Retriever.from_documents(all_docs, k=8)  # was 6
        except Exception as e:
            print(f"[RAG] BM25 init failed: {e}")
            bm25_retriever = None
