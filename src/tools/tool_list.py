...
def build_rag_tool(user_department: str, user_role: str, tenant_id: str = "default"):
    """
    Returns a rag_tool scoped to the calling user and tenant.
    Uses flat retrieval: dense + BM25 ensemble, Flashrank reranking, k=8.
    """
    ...
    def rag_tool(query: str) -> str:
        """
        Retrieves relevant passages from organisational documents that the
        current user is authorised to access, using hybrid (vector + BM25)
        search and Flashrank reranking. Strictly scoped to the user's tenant.
        """
        ...
        # ── Authorised document pool for BM25 ───────────────────────────
        try:
            all_docs = vector_store.similarity_search("", k=500, filter=pgvector_filter)
            print(f"[RAG] Authorised pool: {len(all_docs)} chunks")
        except Exception as e:
            print(f"[RAG] Pool fetch failed: {e}")
        ...
        # ── Dense + BM25 ensemble, k=12 ──────────────────────────────────
        retriever_kwargs = {"k": 12, "filter": pgvector_filter}
...
# ── Static tool list (non-authenticated contexts only) ───────────────────────
_default_rag = build_rag_tool(user_department="General", user_role="HR")
